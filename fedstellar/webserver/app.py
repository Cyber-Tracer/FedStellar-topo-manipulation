...
def calculating_neighbors(graph, root, desired_distance, threshold):
...
def number_efficient(graph, percentage, distance):
    ...
    for node in graph:
        ...
        if len(graph[node]) < threshold:
            connections = connections + calculating_neighbors(graph, node, distance, threshold)

        # if there is still not enough node after adding then add random. this can be changed to anything if desired

        while len(graph[node]) + len(considered) < threshold:
            to_be_added = random.choice(list(graph.keys()))
            print("to be added:")
            print(to_be_added)
...
def incidence_matrix_to_dictionary(matrix):
    converted_matrix = {}
...
def convert_dictionary_to_incidence_matrix(dictionary):
    matrix = []
...
def fedstellar_scenario_deployment_run():
    if "user" in session.keys():
        ...
        if request.is_json:
            ...
            nodes = data['nodes']
            scenario_name = data["scenario_title"] 
            scenario_name += f'_fedstellar_{data["federation"]}_{datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}'
            matrix = data['matrix']
            logging.info(matrix)
            converted_matrix = incidence_matrix_to_dictionary(matrix)
            adjusted_dictionary = number_efficient(converted_matrix,0.5,3)
            reconverted_matrix = convert_dictionary_to_incidence_matrix(adjusted_dictionary)
            logging.info(reconverted_matrix)
