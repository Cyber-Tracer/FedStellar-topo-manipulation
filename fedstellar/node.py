...
class Node(BaseNode):
        ...
        if is_train_set and (self.config.participant["device_args"]["role"] == Role.AGGREGATOR or
            ...
            if self.round is not None:
                ...
                if self.config.participant["device_args"]["role"] == Role.SERVER:
                    # logging.info(f"[NODE.__train_step before  __gossip_model_difusion] self.round:{ self.round }.")
                    # self.__gossip_model_difusion()
                    self.__gossip_model_aggregation()
                    # logging.info(f"[NODE.__train_step after  __gossip_model_difusion] self.round:{ self.round }.")
                else:
                    self.__gossip_model_aggregation()
    ...
    def __on_round_finished(self):
        ...
        if self.round < self.totalrounds:
            logging.info("[NODE] self.round before training : {}".format(self.round))
            self.__train_step()
            logging.info("[NODE] self.round after training : {}".format(self.round))
        else:
